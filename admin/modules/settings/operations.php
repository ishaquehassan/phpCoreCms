...
if(isset($_GET['task'])){
    ...
    if($task == "add"){
        ...
        if(checkPostRequired("value","Value Is Required")){
            $error = true;
        }
        if(checkPostRequired("type","Type Is Required")){
            $error = true;
        }
        ...
        if($error){
            ...
            $insertId = insertDb($connection,$moduleConfigs['db_table'],array(
                ...
                $moduleConfigs['db_prefix']."value" => $_POST['value'],
                $moduleConfigs['db_prefix']."type" => $_POST['type'],
